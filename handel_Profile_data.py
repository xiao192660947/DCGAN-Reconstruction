...
#parser.add_argument("--handel_pixel_value", default=255, help="value to full no profile")  #将其中的二值图像的值更改为想要的值（比如将1改为255等）
parser.add_argument("--data_dim_x",default=128,help="dim of expend 3D data x exis") #期望生成的不全后数据的x维度值
parser.add_argument("--data_dim_y",default=128,help="dim of expend 3D data y exis") #期望生成的不全后数据的x维度值
parser.add_argument("--data_dim_z",default=128,help="dim of expend 3D data z exis") #期望生成的不全后数据的x维度值
...
parser.add_argument("--x_profile_file_path",default="D:/PycharmProjects/Generate_3D_GAN/profile_file/x4_newest.txt",help="filepath for x profile file")#对应的x方向的剖面数据的文件路径
parser.add_argument("--y_profile_file_path",default="D:/PycharmProjects/Generate_3D_GAN/profile_file/y4_newest.txt",help="filepath for x profile file")#对应的y方向的剖面数据的文件路径
parser.add_argument("--z_profile_file_path",default="D:/PycharmProjects/Generate_3D_GAN/profile_file/z4_newest.txt",help="filepath for x profile file")#对应的z方向的剖面数据的文件路径
...
def prepar_profile_data(loc_data_arr,loc_label_arr,complete_profile_data,num_sum):
    ...
    for i in range(0,num_sum):
        ...
        #     loc_label_arr[i] = args.handel_pixel_value
...
if __name__ == "__main__":
    ...
    #进行resize为最小为64的尺寸
    #complete_profile_data = resize(complete_profile_data,[64,70,280])

    print('填充给完毕后的含剖面训练准备数据：', complete_profile_data.shape)
    #save_txt_profile_3Dimage(complete_profile_data,"3")
    save_npy_profile_3Dimage(complete_profile_data,"newestpofile")
