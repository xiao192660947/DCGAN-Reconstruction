...
parser.add_argument("--out_dir",default="D:/PycharmProjects/Generate_3D_GAN/dataset/train_3Dimage/",help='dir of out image')#向相应文件夹中输入数据(一般为test_3Dimage和train_3Dimage# )D:/PycharmProjects/Generate_3D_GAN/dataset/train_label/
#D:/PycharmProjects/Generate_3D_GAN/dataset/train_3Dimage/
parser.add_argument("--input_dir",default="D:/PycharmProjects/Generate_3D_GAN/dataset/profile_data/newestpofile.npy",help='dirs of input image3Ddata')#进行输入的原始数据文件#D:/PycharmProjects/Generate_3D_GAN/fold_categorical_180x150x120.txt
                    ...
                    for m in range(0, out_prepar_data.shape[0]):
                        for n in range(0, out_prepar_data.shape[1]):
                            for s in range(0, out_prepar_data.shape[2]):
                                # if out_prepar_data[m][n][s] == 1:
                                #     f.write('\n' + str(255))
                                # if out_prepar_data[m][n][s] == 0:
                                #     f.write('\n' + str(0))
                                # if out_prepar_data[m][n][s] == 125:
                                #     f.write('\n' + str(125))
                                out_prepar_data[m][n][s] = float(out_prepar_data[m][n][s])
                                out_prepar_data[m][n][s] = int(out_prepar_data[m][n][s])
