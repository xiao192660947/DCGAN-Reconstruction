...
def standardizate_to_inputdim(data_arr,standard_x,standard_y,standard_z,mode = 0):
    ...
    if mode == 1:
        ...
        if x_num < standard_x:
            missing_num = standard_x - x_num
            fill_arr_x = np.zeros([missing_num,min_num,min_num])#用于x轴的补充
            fill_arr_y = np.zeros([standard_x,missing_num,min_num])#用于y轴的补充
            fill_arr_z = np.zeros([standard_x,standard_y,missing_num])#用于z轴的补充
            output_arr = np.concatenate([data_arr,fill_arr_x],axis=0)#补充x轴
            output_arr = np.concatenate([output_arr,fill_arr_y],axis=1)#补充y轴
            output_arr = np.concatenate([output_arr,fill_arr_z],axis=2)#补充z轴
            ...
            output_arr = data_arr[0:standard_x,0:standard_y,0:standard_z]
    print("00" * 100)
    print("输入结果的维度 = ",output_arr.shape)
    print("00" * 100)
    return output_arr
...
def handle_3Ddata(trainning_data_path,label_data_path,trainning_data_name,training_data_format,label_data_format,standard_x,standard_y,standard_z,RGB_data = False):#直接将输入的数据变为单个文件的路径，批量的处理写在外面
    ...
    if RGB_data == False:
        ...
        trainning_data_arr = np.expand_dims(np.array(trainning_data_arr).astype(np.float32), axis=0)
        label_data_arr = np.expand_dims(np.array(label_data_arr).astype(np.float32), axis=0)

    return trainning_data_arr,label_data_arr

'''
读取存放的trainning_3Dimage等npy文件
'''
...
'''
进行数据准备，将小于256的数据扩充为256，大于的数据可以分为多组数据进行处理
可以后面再写，主要针对于扩充训练集
分为两类：
1.关键点较少并且关键点的矢量位置很重要:不能动点，只能将周围的无意义点进行扩充
2.二值连续图像：可以动点就是项filter那样进行分类，最终生成一个数据集array（应该就有4维，三维的数据+一个维度的数量）
'''
